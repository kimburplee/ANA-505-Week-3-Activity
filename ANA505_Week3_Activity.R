...
KimberlyTitanic <- data.frame(Titanic)
...
nrow(KimberlyTitanic)
ncol(KimberlyTitanic)
colnames(KimberlyTitanic)
str(KimberlyTitanic)
...
sex <- KimberlyTitanic[,1]
age <- KimberlyTitanic[,2]
...
new_df <- data.frame(cbind(sex,age))
...
subset <- subset(KimberlyTitanic, Freq>25)
...
ggplot(KimberlyTitanic, aes(Survived, Freq)) +
  geom_col()
